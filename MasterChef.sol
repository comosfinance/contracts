...
// ComosToken with Governance.
contract ComosToken is BEP20('Comos Token', 'COMOS') {
        // Burn address
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
	    ...
	    /// @dev overrides transfer function to meet tokenomics of COMOS
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "COMOS::transfer: Burn value invalid");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "COMOS::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "COMOS::delegateBySig: invalid nonce");
        require(now <= expiry, "COMOS::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "COMOS::getPriorVotes: not yet determined");
...
contract MasterChef is Ownable {
    ...
    // The COMOS TOKEN!
    ComosToken public comos;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public comosPerBlock;
    // Bonus muliplier for early comos makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _comosPerBlock,
        uint256 _startBlock
    ) public {
        comos = _comos;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        comosPerBlock = _comosPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 comosReward = multiplier.mul(comosPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(comosReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 comosReward = multiplier.mul(comosPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        comos.mint(devaddr, comosReward.div(10));
        comos.mint(address(this), comosReward);
        pool.accEggPerShare = pool.accEggPerShare.add(comosReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe comos transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 comosBal = comos.balanceOf(address(this));
        if (_amount > comosBal) {
            comos.transfer(_to, comosBal);
        } else {
            comos.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _comosPerBlock) public onlyOwner {
        massUpdatePools();
        comosPerBlock = _comosPerBlock;
    }
}